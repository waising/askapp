...
import static tcking.github.com.giraffeplayer.R.id.app_video_replay;

/**
 * 播放器
 * Created by tcking on 15/10/27.
 */
public class GiraffePlayer implements IMediaPlayer.OnPreparedListener {
    /**
    ...
    private String smallUrl;//略缩图URl
    ...
    private final View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (v.getId() == R.id.app_video_fullscreen) {//全屏按钮监听
                toggleFullScreen();
            } else if (v.getId() == R.id.app_video_play) {//播放按钮监听
                doPauseResume();
                show(defaultTimeout);
            } else if ((v.getId() == R.id.app_video_replay_icon) || (v.getId() == R.id.app_video_replay)) {//重播
                videoView.setVideoPath(url);
                videoView.seekTo(0);
                start();
                doPauseResume();
            } else if (v.getId() == R.id.app_video_finish) {//回退按钮
                if (!fullScreenOnly && !portrait) {
                    activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                } else {
                    activity.finish();
                }
            } else if (v.getId() == R.id.re_load)//重新加载网络.再判断是否有网络
            {
                if (reloadNetWorkListener != null) {
                    reloadNetWorkListener.netWork();
                }
            }
        }
    };
    ...
    public void doPauseResume() {
        if (status == STATUS_COMPLETED) {//如果播完了
            $.id(app_video_replay).gone();
            videoView.seekTo(0);
            start();
        } else if (videoView.isPlaying()) {//如果正在播放
     ...
     * 无网络提示消失
     */
    public void noNetWorkGone() {
        $.id(R.id.ll_network).gone();
...
//        });
    ...
    private final SeekBar.OnSeekBarChangeListener mSeekListener = new SeekBar.OnSeekBarChangeListener() {
        @Override
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {//监听用户触发情况，正在拖动情况
            if (!fromUser)//如果不是用户触发，下面函数不执行
                return;
        ...
        public void onStopTrackingTouch(SeekBar seekBar) {//停止拖动
            ...
            if (progressListener != null) {//停止拖动的时候，监听
                progressListener.progress(totalSeconds(position), totalSeconds(duration));
            }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MESSAGE_SEEK_NEW_POSITION:
                    ...
                    break;
                case MESSAGE_SHOW_PROGRESS://显示进度条内容
                    setProgress();
                    ...
                    break;
                case MESSAGE_RESTART_PLAY://重新播放
                    play(url);
                    break;
            }
        }
    ...
    public GiraffePlayer(final Activity activity) {
        ...
        videoView.setOnCompletionListener(new IMediaPlayer.OnCompletionListener() {//播放完成监听
            @Override
        ...
        videoView.setOnErrorListener(new IMediaPlayer.OnErrorListener() {//错误监听
            @Override
        ...
        videoView.setOnInfoListener(new IMediaPlayer.OnInfoListener() {//在有警告或错误信息时调用。例如：开始缓冲、缓冲结束、下载速度变化。
            @Override
...
//                        Toaster.show("download rate:" + extra);
        ...
        seekBar.setMax(1000);
        seekBar.setOnSeekBarChangeListener(mSeekListener);//seekBar监听
        $.id(R.id.app_video_play).clicked(onClickListener);//播放按钮监听
        $.id(R.id.app_video_fullscreen).clicked(onClickListener);//全屏按钮监听
        $.id(R.id.app_video_finish).clicked(onClickListener);//顶部回退监听
        $.id(R.id.app_video_replay_icon).clicked(onClickListener);//重播监听
        $.id(R.id.re_load).clicked(onClickListener);//网络监听
        $.id(app_video_replay).clicked(onClickListener);//重播监听
        ...
        liveBox.setOnTouchListener(new View.OnTouchListener() {//手势监听
            @Override
            public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        orientationEventListener = new OrientationEventListener(activity) {//横竖屏监听
            @Override
    ...
    public void setUrl(String url) {
        ...
        if (playerSupport) {
            videoView.setVideoPath(url);
            $.id(app_video_replay).visible();//中间播放按钮可见
        }
    }
     ...
     * 试看结束
     */
    public void trySeeFinish() {
        $.id(R.id.iv_try_see_finish).visible();
        pause();
        liveBox.setOnTouchListener(new View.OnTouchListener() {//禁用触摸事件，防止向子view传播事件
            @Override
            public boolean onTouch(View view, MotionEvent motionEvent) {
                return true;
            }
...
//        });
    ...
    private long totalSeconds(long time) {
...
//        }
    ...
    public void noBuffer() {
        videoView.setmCurrentBufferPercentage(0);
    }
    ...
    @Override
    public void onPrepared(IMediaPlayer mp) {//视频播放进度监听
        if (preparedListener != null) {
            preparedListener.prepare(videoView.getDuration());
        }

        mp.setOnBufferingUpdateListener(new IMediaPlayer.OnBufferingUpdateListener() {//缓冲监听
            @Override
            public void onBufferingUpdate(IMediaPlayer mp, int percent) {
                long position = videoView.getCurrentPosition();//当前播放到哪里
                long duration = videoView.getDuration();//总时长
                if (progressListener != null) {
                    progressListener.progress(totalSeconds(position), totalSeconds(duration));
                }
            }
    ...
    public class PlayerGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        @Override
        public boolean onSingleTapUp(MotionEvent e) {//在按下并抬起时发生
            if (isShowing) {
                hide(false);
            } else {
                show(defaultTimeout);
    ...
    public void seek(int progress, long totalTime) {
        videoView.seekTo((int) ((totalTime * progress * 1.0) / 100));
    }
    ...
    public void setProgressListner(ProgressListener progressListner) {

        this.progressListener = progressListner;
    }

    public interface PreparedListener {
        void prepare(long totalTime);
    }
     ...
     * 网络监听
     */
    public interface ReloadNetWorkListener {
        void netWork();
    }

    private ReloadNetWorkListener reloadNetWorkListener;

    public void setReloadNetWorkListener(ReloadNetWorkListener reloadNetWorkListener) {
        this.reloadNetWorkListener = reloadNetWorkListener;
    }
    ...
    public void setBackGroundWhite(Context context) {
        videoView.setBackgroundColor(context.getResources().getColor(R.color.white));
    }
}
